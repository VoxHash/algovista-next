...
const Pill: React.FC<React.PropsWithChildren> = ({ children }) => (
  <span className="px-2 py-1 rounded-full bg-gradient-to-r from-indigo-500/10 to-fuchsia-500/10 text-indigo-300 text-xs border border-indigo-400/20">
    {children}
  </span>
);
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="w-36"><Slider value={[speed]} min={10} max={800} step={10} onValueChange={(v)=>setSpeed(v[0])}/></div>
      </div>
      {typeof size === 'number' && setSize && (
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-400">Size</span>
          <div className="w-36"><Slider value={[size]} min={5} max={80} step={1} onValueChange={(v)=>setSize(v[0])}/></div>
        </div>
      )}
    </div>
  );
...
function SortingPanel() {
  ...
  const bars = arr.map((v, i) => {
    ...
    const isRange = (highlight as any).range && i >= (highlight as any).range[0] && i <= (highlight as any).range[1];
    const hue = isA || isB ? 'bg-fuchsia-400' : isRange ? 'bg-indigo-400/70' : 'bg-slate-300/70';
    return (
      <motion.div key={i} layout className={`rounded-t ${hue}`} style={{ height: v*2 }} />
    );
  });
    ...
    <Card className="bg-gradient-to-b from-slate-900 to-slate-950 border-slate-800">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <a href="https://github.com/CryptoJoma" target="_blank" rel="noreferrer">
      ...
      <CardContent className="space-y-4">
        <ControlBar
          ...
          setSize={setSize}
        />
        <div className="h-64 flex items-end gap-1 p-2 rounded-xl bg-slate-900/60 border border-slate-800 overflow-hidden">
          <AnimatePresence>{bars}</AnimatePresence>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800">
            <div className="text-xs text-slate-400 mb-2">Complexity</div>
          ...
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800">
            <div className="text-xs text-slate-400 mb-2">Writes per step</div>
            <div className="h-24">
          ...
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800">
            <div className="text-xs text-slate-400 mb-2">Current Event</div>
...
function PathfindingPanel() {
  ...
  for (let y=0;y<rows;y++) for (let x=0;x<cols;x++) {
    ...
    let cls = 'bg-slate-800/60 border-slate-700/60';
    if (isWall) cls = 'bg-slate-700/90 border-slate-600';
    if (visited) cls = 'bg-indigo-700/60 border-indigo-500/40';
    if (relaxed) cls = 'bg-fuchsia-700/60 border-fuchsia-500/40';
    if (inPath) cls = 'bg-emerald-600/70 border-emerald-400/50';
    if (isStart) cls = 'bg-blue-600/80 border-blue-400/60';
    if (isGoal) cls = 'bg-pink-600/80 border-pink-400/60';
    cells.push(<div key={`${x}-${y}`} onClick={()=>toggleWall(x,y)} className={`cursor-pointer border ${cls} rounded-sm`} />);
  }
  ...
  return (
    <Card className="bg-gradient-to-b from-slate-900 to-slate-950 border-slate-800">
      <CardHeader className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-3">
          <Route className="h-5 w-5 text-fuchsia-400"/>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800 space-y-2">
            <div className="text-xs text-slate-400">Grid Size</div>
            <div className="flex items-center gap-2"><Label className="text-xs">Rows</Label><div className="w-36"><Slider value={[rows]} min={8} max={24} step={1} onValueChange={(v)=>setRows(v[0])}/></div></div>
            <div className="flex items-center gap-2"><Label className="text-xs">Cols</Label><div className="w-36"><Slider value={[cols]} min={12} max={36} step={1} onValueChange={(v)=>setCols(v[0])}/></div></div>
          </div>
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800">
          ...
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800 md:col-span-2">
            <div className="text-xs text-slate-400 mb-2">Complexity</div>
...
function KMPPanel() {
  ...
  const reset = () => { setEvents([]); setMatches([]); setGen(null); setRunning(false); setLps([]); setPos({i:-1,j:-1}); };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800 md:col-span-2">
            ...
            <div className="flex flex-col gap-2">
              <input className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-100" value={text} onChange={(e)=>{ setText(e.target.value); reset(); }} />
              <input className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-100" value={pat} onChange={(e)=>{ setPat(e.target.value); reset(); }} />
            </div>
            <div className="mt-3 text-sm text-slate-200 font-mono break-words">
              {text.split("").map((ch, idx)=>{
                const active = idx===pos.i;
                const hit = matches.includes(idx);
                return <span key={idx} className={`${active?'bg-indigo-600/60':''} ${hit?'underline decoration-emerald-400':''} px-0.5`}>{ch}</span>
              })}
            </div>
          ...
          <div className="rounded-xl p-3 bg-slate-900/60 border border-slate-800">
            <div className="text-xs text-slate-400 mb-2">LPS Table</div>
            <div className="flex flex-wrap gap-1 text-sm font-mono">
              {pat.split("").map((c,i)=> (
                <div key={i} className="px-2 py-1 rounded bg-slate-800 border border-slate-700 flex items-center gap-2">
                  <span className="text-slate-300">{c}</span>
                  <span className="text-emerald-400">{lps[i] ?? "·"}</span>
                </div>
              ))}
            </div>
            <div className="mt-3 text-xs text-slate-400">Complexity: Best/Avg/Worst {BIGO.kmp.best} | Space {BIGO.kmp.space}</div>
          </div>
        </div>
      </CardContent>
    </Card>
      ...
      <header className="sticky top-0 z-10 backdrop-blur supports-[backdrop-filter]:bg-slate-950/60 border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold tracking-tight">AlgoVista — Algorithm Visualizer</h1>
...
function FooterCard() {
  return (
    <Card className="bg-gradient-to-b from-slate-900 to-slate-950 border-slate-800">
      <CardContent className="py-4 text-xs text-slate-400 flex flex-wrap items-center justify-between">
        <div>
          © 2025 AlgoVista — Built with Next.js, Tailwind, Framer Motion, Recharts.
        </div>
        <div className="flex items-center gap-2">
          <a className="underline-offset-4 hover:underline" href="https://github.com/VoxHash" target="_blank" rel="noreferrer">GitHub</a>
          <a className="underline-offset-4 hover:underline" href="https://youtube.com/@VoxHash" target="_blank" rel="noreferrer">YouTube</a>
        </div>
      </CardContent>
    </Card>
  );
